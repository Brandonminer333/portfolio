export const metadata = {
  ...
  description:
    "A case study on the impacts of Random Forest prediction aggregation: majority, weighted, and ranked voting.",
};
...
<nav aria-label="Project navigation" className="not-prose mb-6 flex flex-wrap gap-3">
  <a href="https://github.com/Brandonminer333/random_forest_voting" target="_blank" rel="noopener noreferrer" className="opacity-80 hover:opacity-100">GitHub</a>
</nav>

## Introduction

This discussion focuses on the machine-learning model known as a **Random Forest**, an ensemble technique built from many **decision trees**. A decision tree is a model that recursively splits the data using a greedy algorithm to form increasingly homogeneous subsets. While decision trees are intuitive and powerful, they tend to **overfit**. This means that they generalize poorly.
...
This is the most familiar aggregation method. Each tree predicts a class; the forest outputs the **mode** of these predictions.
...
Majority voting elects class $A$, because two trees predict $A$. But weighted voting notes that Tree 3 has a stronger signal — its leaf has more samples, and those samples unanimously support $B$.

In weighted voting, each tree's vote is weighted by the **class probability** (or the proportion of samples in the leaf). Predictions with higher confidence influence the final output more.
...
This result helps explain why **scikit-learn's RandomForestClassifier uses weighted aggregation internally**: it tends to be the most statistically reliable method, especially when tree structures are constrained.

## Regression

Random Forest **regression** is different. Each leaf node returns the **mean** of the target values in that leaf. The forest prediction is simply the **mean of the tree means**.
...
- **statistically inferior** (more biased or higher variance)
